                    ...
                    <div class="col-12 p-0 mt-4">
                        <div class="card">
                            <div class="card-header border-bottom">
                                <h6 class="text-primary" style="font-size: 16px">Edit Category</h6>
                            </div>
                            <div class="card-body">

                                {{-- <form method="POST" id="category-form"
                                    action="{{ route('admin.updateCategory', $data->id) }}" class="mx-3 p-3"
                                    enctype="multipart/form-data"> --}}
                                <form method="POST" id="category-form"
                                    onsubmit="uploadData1('category-form','{{ route('admin.updateCategory', $data->id) }}', 'alert-box', 'btn-box-1', event)"
                                            ...
                                            <div class="border border-dashed p-3">
                                                <div class="selected-media-box" id="selected-media-box">
                                                    <input type="hidden" id="final-selected-media-input"
                                                        name="category_image" value="{{ $data->cat_img }}">
                                            ...
                                            <div class="border border-dashed p-3">
                                                <div class="selected-media-box" id="selected-media-box2">
                                                    <input type="hidden" id="final-selected-media-input2"
                                                        name="category_banner" value="{{ $data->cat_banner }}">

                                                    @if (!is_null($data->cat_banner) && $data->cat_banner != 'null')
                                                        @php
                                                            $cat_banner = json_decode($data->cat_banner, true);
                                                            echo getMediaFile($cat_banner[0]['file_id']);
                                                        @endphp
                                                    @endif
